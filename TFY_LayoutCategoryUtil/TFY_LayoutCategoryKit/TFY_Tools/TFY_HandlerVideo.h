 ...
 */
- (void)tfy_composesVideoFullPath:(NSString *)videoFullPath frameImgs:(NSArray<UIImage *> *)frameImgs fps:(int32_t)fps progressImageBlock:(CompProgressBlcok)progressImageBlock completedBlock:(CompCompletedBlock)completedBlock;

/**
 *  多个小视频合成大视频 subsectionPaths 视频地址数组 videoFullPath 合成视频路径 isHaveAudio 视频是有声音，YES:创建音频轨道 NO:不创建音频轨道 completedBlock 完成回调
 */
- (void)tfy_combinationVideosWithVideoPath:(NSArray<NSString *> *)subsectionPaths videoFullPath:(NSString *)videoFullPath isHavaAudio:(BOOL)isHaveAudio progressBlock:(CompProgressBlcok)progressBlock completedBlock:(CompFinalCompletedBlock)completedBlock;
